...
class report: UIViewController, UITableViewDelegate,UITableViewDataSource, MFMailComposeViewControllerDelegate {
    ...
    func alert17(){
        let alertController17 = UIAlertController(title: ("Tax calaculation") , message: "Tax accural calculation is based on invoice cancellation timing(if occured) and therefore tax allocation can be different from this report. Tax filing should be based on 'Tax' and Not 'Reports'", preferredStyle: .alert)
